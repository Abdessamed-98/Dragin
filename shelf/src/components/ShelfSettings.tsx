...
interface Props {
  shelf: Shelf;
  onRename: (shelfId: string, newName: string) => void;
  onToggleAutoPin: (shelfId: string, autoPin: boolean) => void;
  onDelete: (shelfId: string) => void;
  onClose: () => void;
}

export function ShelfSettings({ shelf, onRename, onToggleAutoPin, onDelete, onClose }: Props) {
  const [name, setName] = useState(shelf.name);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const isGeneral = shelf.name === 'General';

  const handleRename = () => {
    const trimmed = name.trim();
    if (trimmed && trimmed !== shelf.name) {
      onRename(shelf.id, trimmed);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-lg font-semibold mb-4">Shelf Settings</h2>
        ...
        <div className="flex items-center justify-between py-3 border-t border-slate-700">
          ...
          <button
            onClick={() => onToggleAutoPin(shelf.id, !shelf.autoPin)}
          ...
          >
            {shelf.autoPin ? <Pin className="w-4 h-4" /> : <PinOff className="w-4 h-4" />}
          </button>
        </div>
          ...
          <div className="pt-3 border-t border-slate-700 mt-3">
            {confirmDelete ? (
              <div className="flex items-center gap-2">
                <span className="text-xs text-red-400">Delete this shelf?</span>
                <button
                  onClick={() => { onDelete(shelf.id); onClose(); }}
